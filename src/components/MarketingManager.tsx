...
import PurchaseAPISettings from './marketing-manager/PurchaseAPISettings';
...
const MarketingManager: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (hash.includes('marketing-manager-')) {
      ...
      } else if (tab === 'purchase-api-settings') {
        setActiveTab('purchase-api-settings');
      } else {
  ...
  const renderPurchaseAPISettingsContent = () => {
    return (
      <div className="space-y-8">
        <PurchaseAPISettings />
      </div>
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'purchase-api-settings':
        return renderPurchaseAPISettingsContent();
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-100">
        <div className="border-b border-gray-200">
          <nav className="flex space-x-8 px-6" aria-label="Marketing Manager Tabs">
            ...
            <button
              onClick={() => handleTabChange('purchase-api-settings')}
              className={`py-4 px-1 border-b-2 font-medium text-sm transition-colors ${
                activeTab === 'purchase-api-settings'
                  ? 'border-blue-500 text-blue-600'
                  : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
              }`}
            >
              Purchase API Settings
            </button>
            <button
