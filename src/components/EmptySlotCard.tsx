...
export default function EmptySlotCard({ onClick }: EmptySlotCardProps) {
  return (
    <div onClick={onClick} className="relative w-60 h-[262px] flex-shrink-0 cursor-pointer">
      {/* gradient spinning border layer */}
      <div className="absolute inset-0 rounded-xl animate-spin-slow [background:conic-gradient(var(--tw-gradient-stops))] from-amber-400 via-pink-500 to-amber-400" style={{ animationDuration: '6s' }} />

      {/* inner card */}
      <div className="relative z-10 w-full h-full m-[2px] rounded-[calc(theme(borderRadius.xl)-2px)] bg-black/90 border-2 border-dashed border-white/20 flex items-center justify-center">
        {/* 20% flag */}
