...
interface SalesBannerDesktopProps {
...
export default function SalesBannerDesktop({ 
  beforeCouponText, 
  afterCouponText, 
  couponCode 
}: SalesBannerDesktopProps) {
  ...
  return (
    <div className="hidden sm:flex items-center justify-center gap-1 text-center relative">
      {/* Before coupon text */}
      ...
      >
        <span className="font-bold text-[0.875rem]">{couponCode}</span>
      ...
      <TextWithPlaceholders 
        ...
        className="text-[0.875rem] font-bold text-black ml-1"
      />

      {/* Copy success message */}
      {copySuccess && (
        <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-2 bg-black text-[#59e3a5] px-3 py-1 rounded-md text-[0.75rem] font-medium shadow-lg z-10">
          🎉 Copied to clipboard!
        </div>
      )}
    </div>
