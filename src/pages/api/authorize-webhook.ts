...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.log('🔔 WEBHOOK: Transaction data:', JSON.stringify(transactionData, null, 2));
        ...
        // Handle FRAUD-RELATED payment failure events
    // NOTE: Regular payment declines (insufficient funds, expired cards, etc.) are handled
    // via iframe communication in checkout.tsx. Webhooks are only used for fraud-related declines.
    if (eventType === 'net.authorize.payment.fraud.declined' || 
        eventType === 'net.authorize.payment.fraud.held') {

      console.log('🔔 WEBHOOK: FRAUD-related payment failed, sending failure emails...');
