...
export default function CheckoutPage() {
  ...
  // Process payment with our backend using opaque data
  const processAcceptPayment = async (opaqueData: any) => {
      console.log('Processing payment with opaque data...', opaqueData);
      ...
      try {
          ...
          console.log('Calling /api/process-payment-accept...');
          const response = await fetch('/api/process-payment-accept', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          if (result.success) {
              // Call original handleSuccessfulPayment with the transaction details
  ...
  useEffect(() => {
      (window as any).responseHandler = (response: any) => {
        console.log('Accept.js Response:', response);
        if (response.messages.resultCode === "Error") {
          console.error('Accept.js Error:', response.messages.message);
          const errorText = response.messages.message.map((m: any) => `${m.code}: ${m.text}`).join('\n');
          setFormError('Payment Verification Failed: ' + errorText);
          setIsLoading(false);
          setShowProcessingPopup(false);
        } else {
          console.log('Accept.js Success:', response.opaqueData);
          processAcceptPayment(response.opaqueData);
        }
      };
  }, [billingData, orderItems, addOnOrderItems, total, currentUser, formData]); // Dependencies needed for processAcceptPayment

  const dispatchPayment = () => {
    const authData = {
      clientKey: process.env.NEXT_PUBLIC_AUTHORIZE_NET_CLIENT_KEY,
      apiLoginID: process.env.NEXT_PUBLIC_AUTHORIZE_NET_LOGIN_ID || process.env.AUTHORIZE_NET_API_LOGIN_ID
    };
    ...
    if (typeof (window as any).Accept === 'undefined') {
        setFormError('Payment system not loaded. Please refresh the page.');
        setIsLoading(false);
        setShowProcessingPopup(false);
        return;
    }

    console.log('Dispatching to Accept.js...');
    // Use string name of global function as required by some versions of Accept.js
  ...
  const handlePaymentSubmit = async () => {
      ...
      if (!cardNumber || cardNumber.replace(/\s/g, '').length < 13) {
          ...
          // Scroll to error
          const element = document.querySelector('.bg-red-500\\/10');
  ...
  const handlePaymentSubmitOld = async () => {
    try {
